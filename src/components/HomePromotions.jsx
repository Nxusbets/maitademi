...
import './HomePromotions.css'; // Asegúrate de crear este archivo
...
const HomePromotions = () => {
  ...
  return (
    <section className="promotions-section-main">
      <h2 className="promotions-title">Promociones</h2>
      <div className="promotions-list">
        {promotions.map((promo) => (
          <div
            key={promo.id}
            className={`promotion-card ${promo.status === 'inactive' ? 'promotion-inactive' : ''}`}
          >
            <span className="promotion-badge">PROMOCIÓN</span>
            <h3 className="promotion-card-title">
              {promo.title || promo.titulo || 'OFERTAS DE TEMPORADA'}
            </h3>
            <div className="promotion-code">
              <span>
                Código: <strong>{promo.codigo || promo.code || ''}</strong>
              </span>
            </div>
            <p className="promotion-description">
              {promo.message || promo.descripcion || ''}
            </p>
            {promo.discount && (
              <div className="promotion-discount">
                <span>Descuento: {promo.discount}%</span>
              </div>
            )}
            {promo.validUntil && (
